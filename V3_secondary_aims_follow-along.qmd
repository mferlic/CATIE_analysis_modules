...
This is data that was *simulated* to mimic data arising from the ADHD SMART study (PI: William Pelham).
...
## Step 1: moderators of stage 2 treatment
...
We test this secondary aim by fitting a **moderated regression model** using data from non-responders.
...
We will use a very powerful package called `emmeans` to estimate the marginal mean of end-of-year school performance under different treatment/covariate options.
...
The results suggest that for those children who are non-adherent (`adherence` = 0) it is better to Augment ($A2 = -1$) rather than intensify. Likewise, for those children that are adherent (`adherence` = 1) it is better to Intensify ($A2=1$). Notice, our optimal tactic does not depend on first-stage treatment $A1$.
...
## Step 3: first-stage tailoring controlling for optimal second-stage
...
::: callout-warning
The standard errors here are potentially incorrect because they don't take into account sampling error in estimation of $Y^{opt}$. Luckily, we provide software (see `qlaci`) to do proper inference!
:::
...
To estimate the mean outcome if every child had received the more deeply-tailored adaptive intervention we essentially repeat step 2 and create a new data frame with the optimal first-stage tactic by levels of `priormed`.
...
First, we specify the contrast matrix that will be used for the stage 1 regression (step 3 of Q-learning).
...
With valid confidence intervals we can test contrasts and even compare to the 4 embedded adaptive interventions found in Virtual Module 2!
