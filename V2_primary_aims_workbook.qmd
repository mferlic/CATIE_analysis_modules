...
dat_smart <- dat_smart %>%
  ...
  mutate(odd_c = odd - mean(odd),                 # this creates X_1c
         ...
         priormed_c = priormed - mean(priormed),  # this creates X_3c
         race_c = race - mean(race),              # this creates X_4c
         Y0_c = Y0 - mean(Y0))                    # this creates Y0_c         
```
...
## Analysis Results for Module 5

We can obtain valid estimates of the causal effects using any subset of covariates. Below, we show the results if we use one covariate or no covariates. These results exactly match those shown in Module 5.

This first analysis shows the results if we filter down to participants consistent with AI#1 or AI#2.
First, we'll use `Y0` as a covariate.
...
```{r}
L <- rbind(
  "AI#1 (MED, AUGMENT)"    = c(1, -1, 0),
  "AI#2 (BMOD, AUGMENT)"   = c(1,  1, 0),
  "AI#1 vs. AI#2" = c(0, -2, 0)
)

est_contrasts <- estimate(fit = model, combos = L)
print(est_contrasts)
```
Now we perform the same analysis without covariates.
...
```{r}
est_contrasts <- estimate(fit = model, combos = L[,1:2])
print(est_contrasts)
```

This second approach analyzes all four AIs at the same time.
First, we fit a model using `Y0` as a baseline covariate.

```{r}
model <- geeglm(Y2 ~ A1 + A2 + I(A1*A2) + Y0_c,
                # specify which column contains the participant ID's
                id = ID,  
                # remember to use the weighted and replicated dataset
                data = dat_smart_replicated,  
...
```{r}
L <- rbind(
  "AI#1 (MED, AUGMENT)"    = c(1, -1, -1,  1, 0),
  "AI#2 (BMOD, AUGMENT)"   = c(1,  1, -1, -1, 0),
  "AI#3 (MED, INTENSIFY)"  = c(1, -1,  1, -1, 0),
  "AI#4 (BMOD, INTENSIFY)" = c(1,  1,  1,  1, 0),
  "AI#1 vs. AI#2" = c(0, -2, 0, 2, 0),
  "AI#1 vs. AI#3" = c(0, 0, -2, 2, 0),
  "AI#1 vs. AI#4" = c(0, -2, -2, 0, 0)
)
...
Then we fit another model with no covariates.
...
```{r}
est_contrasts <- estimate(fit = model, combos = L[,1:4])
print(est_contrasts)
```
