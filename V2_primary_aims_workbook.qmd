...
dat_smart_replicated <- rbind(dat_rows_not_to_replicate,
  ...
  arrange(ID) # crucial that we arrange dataset by ID in order for geeglm to properly identify the "clusters"
```
...
```{r}
# we arrange by ID so that geeglm properly accounts for the duplicated units. See ?geeglm for details
